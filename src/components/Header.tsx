  ...
  return (
    ...
    >
      <nav className="container mx-auto px-6 py-4">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden mt-4 p-4 bg-card backdrop-blur-sm rounded-lg shadow-lg border border-border animate-fade-in">
            {['home', 'servicos', 'portfolio', 'depoimentos', 'contato'].map((item) => (
              <button
                key={item}
                onClick={() => scrollToSection(item)}
                className="block w-full text-left px-4 py-3 text-card-foreground hover:text-primary hover:bg-accent rounded-md transition-all duration-200"
              >
                {item.charAt(0).toUpperCase() + item.slice(1)}
              </button>
            ))}
